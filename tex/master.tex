...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Procedure{DodajUČvor}{objekti, čvor, $Parovi$}
		\If{čvor nije list}
			\State DodajNaniže(obj, čvor, $Parovi$)
			\State \textbf{return}
		\EndIf		

		\For{element in elementi}
			\If{obj i element imaju presek}
				\State dodaj par (obj, element) u kolekciju $Parovi$
			\EndIf		
		\EndFor

		\State dodaj obj u čvor.elementi
		...
		\Procedure{DodajNaniže}{obj, čvor, $Parovi$}
		\State $Parovi := \{ \}$
		\For{podoktant in čvor}
			\If{podoktant i obj imaju presek}
				\State DodajUČvor(obj, podoktant, $Parovi$)
			\EndIf	
		\EndFor
		\State \textbf{return} $Parovi$
		\EndProcedure
...
\begin{algorithm}
	\caption{Briši i odseci algoritam detekcije kolizije todo}
    \label{alg:triv}
	\begin{algorithmic}[1]
		\Procedure{OsnovnaDetekcijaKolizije}{objekti}%\Comment{elements je niz svih objekata}
		\State $Parovi := \{ \}$
