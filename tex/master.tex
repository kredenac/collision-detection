...
naći sve parove objekata koji se presecaju.
Nekada se dodatno traži i oblik preseka ili precizan trenutak početka preseka. 
...
\begin{algorithm}
    ...
    \label{alg:octree}
	\begin{algorithmic}[1]
		\Procedure{OktriNađiKolizije}{objekti}%\Comment{elements je niz svih objekata}
		\State $Parovi := \{ \}$
		\For{obj in objekti}
		...
		\Procedure{DodajUČvor}{objekat, čvor, $Parovi$}
		\If{čvor nije list}
		...
		\For{element in elementi}
			\If{objekat i element imaju presek}
		...
		\If{čvor na maksimalnoj dubini ili broj(čvor.elementi) <= maksimum}
			\State \textbf{return}
		\EndIf	

		\State podeli čvor na podoktante

		\For{element in čvor.elementi}
		...
		\Procedure{DodajNaniže}{objekat, čvor, $Parovi$}
		\For{podoktant in čvor}
			\If{podoktant i obj imaju presek}
	...
	\begin{algorithmic}[1]
		...
		\For{i:=1 to 2*osa.brojTačaka}
			...
			\While{j >= 0 and osa[j].vrednost > vrednost}
				...
				\If{pomeriti je početna i trenutna je krajnja tačka projekcije}

					\Comment{$preseci$ je struktura koja čuva parove objekata koji imaju presek}
					\State ukloni par (pomeriti, trenutna) iz $preseci$
				\EndIf		

				\If{pomeriti je krajnja i trenutna je početna tačka projekcije}
					\If{$objekti$[pomeriti.indeks] i $objekti$[trenutna.indeks] se seku}
...
Za manji broj objekata on je višestruko bolji od prehtodno prikazanog (trivijalnog) algoritma.
...
I za manji broj objekata je višestruko bolji u odnosu na prethodno prikazani, pada 
...
pošto algoriam nije linearne vremenske složenosti.
...
Ovaj efekat prikazan je na slici \ref{fig:octreSpike} i mogu se uočiti tri manja plava skoka za oktri bez duplikata, i dva narandžasta skoka reda veličine za oktri sa duplikatima.
...
Pokazuje se da sa porastom vrednosti maksimalnog broja elemenata u listu relativan dt opada sve do vrednosti 32, od koje počinje da raste.
Najbolje performanse algoritam postiže za vrednost 32, kada je broj slika u sekundi 94 što je čak 36.2\% bolje (u odnosu na prethodnih 69fps)! 
Ubrzanje od 36.2\% je u praksi veoma značajno, pogotovo što je postignuto samo promenom jednog koeficijenta.
...
sortiran, pa je potrebno izvesti dodatne zamene elemenata u nizu dok on ponovo postane sortiran. 
...
a vreme izvršavanja eksplodira, čak ispod jednog iscrtavanja u sekundi.
...
Kao što povećanje brzine kretanja može imati jako negativne posledice na performanse SAP algoritma, tako i smanjivanje brzine utiče povoljno na njegovo vreme izvršavanja.
...
Prvo merenje je izvršeno pri podrazumevanoj brzini, drugo pri duplo manjoj i treće pri četri puta manjoj brzini.
...
\begin{figure}[h!]
	...
	\caption{Upoređivanje rasta relativnog dt u zavisnosti od broja objekta pri njihovim različitim brzinama - SAP.}
...
što se može videti na \ref{fig:drawVsNoDraw}, jer za oko 50 000 objekata relativan dt prelazi 2. 
Ovde je SAP višestruko bolji od ostalih, dostižući 60fps tek na 75 000, a 30fps na 92 000 objekata.
Pritom ostaje ispod referentne prave koja označava linearnu zavisnost, sve do momenta kada se približava broju objekata koji dovodi 
...
\begin{figure}[h!]
	...
	\caption{Relativan dt u zavisnosti od broja objekta kada se objekti sporo kreću (64 puta sporije od podrazumevane brzine) - SAP.}
	\label{fig:sap_great}
\end{figure}
